import React, { useEffect, useState } from 'react';
import { AlertTriangle, Sparkles, RefreshCw, Eye } from 'lucide-react';
import { Button } from '../components/ui/Button';
import { Badge } from '../components/ui/Badge';
import { Link } from 'react-router-dom';
import api from '../api/client';

export const FailedPayments: React.FC = () => {
  const [payments, setPayments] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetchFailed();
  }, []);

  const fetchFailed = async () => {
    try {
      setLoading(true);
      const res = await api.get('/payments/failed');
      setPayments(res.data.data || []);
    } catch (err) {
      console.error('Failed to load failed payments:', err);
    } finally {
      setLoading(false);
    }
  };

  const formatAmount = (paise: number) => `₹${(paise / 100).toLocaleString('en-IN')}`;

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        <div>
          <h1 className="text-xl font-bold text-slate-900 tracking-tight flex items-center">
            Failed Payments Management
            <span className="ml-2.5 px-2 py-0.5 text-xs font-semibold bg-rose-50 text-rose-700 rounded-full border border-rose-200">
              Direct Failures
            </span>
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Isolated failure events awaiting automated recovery ingestion or human intervention.
          </p>
        </div>

        <Button variant="secondary" size="sm" icon={RefreshCw} onClick={fetchFailed} isLoading={loading}>
          Refresh
        </Button>
      </div>

      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200 text-slate-500 font-semibold uppercase tracking-wider">
                <th className="py-3.5 px-4">Payment ID</th>
                <th className="py-3.5 px-4">Customer</th>
                <th className="py-3.5 px-4">Amount</th>
                <th className="py-3.5 px-4">Failure Code / Type</th>
                <th className="py-3.5 px-4">Reason Description</th>
                <th className="py-3.5 px-4">Method</th>
                <th className="py-3.5 px-4 text-right">Inspect</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr>
                  <td colSpan={7} className="text-center py-12 text-slate-400">
                    <RefreshCw className="w-5 h-5 animate-spin mx-auto mb-2 text-sky-500" />
                    Loading failed payments...
                  </td>
                </tr>
              ) : payments.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-12 text-slate-400">
                    No failed payments recorded.
                  </td>
                </tr>
              ) : (
                payments.map((p) => (
                  <tr key={p._id} className="hover:bg-slate-50/60 transition-colors">
                    <td className="py-3.5 px-4 font-mono text-[11px] text-slate-600">
                      {p.razorpayPaymentId || p._id.slice(-8)}
                    </td>
                    <td className="py-3.5 px-4 font-semibold text-slate-900">
                      {p.customerId?.name || 'Customer'}
                    </td>
                    <td className="py-3.5 px-4 font-bold text-rose-600">
                      {formatAmount(p.amount)}
                    </td>
                    <td className="py-3.5 px-4">
                      <span className="font-semibold text-slate-800">{p.failureType || 'UNKNOWN'}</span>
                    </td>
                    <td className="py-3.5 px-4 text-slate-500 truncate max-w-xs">
                      {p.failureReason || 'Declined'}
                    </td>
                    <td className="py-3.5 px-4 uppercase text-slate-600 font-medium">
                      {p.method || 'UPI'}
                    </td>
                    <td className="py-3.5 px-4 text-right">
                      <Link to={`/payments/${p._id}`}>
                        <Button variant="ghost" size="sm" icon={Eye}>
                          Details
                        </Button>
                      </Link>
                    </td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
};
