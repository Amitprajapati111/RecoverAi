 ...
 */
class MockAIProvider extends AIProvider_1.AIProvider {
    constructor() {
        super(...arguments);
    ...
    async analyzeForRecovery(context) {
        ...
        const requiresApproval = context.payment.amount > context.policy.requireApprovalAboveAmount;
        return {
    ...
    calculateRecoveryScore(ctx) {
        let score = 0.5; // base
        // Customer history is the strongest signal
        const successRate = ctx.customer.totalPayments > 0
        ...
        const recoverableTypes = [
            constants_1.FAILURE_TYPE.BANK_TIMEOUT,
            constants_1.FAILURE_TYPE.UPI_TIMEOUT,
            constants_1.FAILURE_TYPE.NETWORK_ERROR,
        ];
        const hardFailures = [
            constants_1.FAILURE_TYPE.CARD_EXPIRED,
            constants_1.FAILURE_TYPE.INSUFFICIENT_FUNDS,
            constants_1.FAILURE_TYPE.BANK_DECLINED,
        ];
        ...
        if (ctx.recoveryCase.attemptCount >= 2)
            score -= 0.2;
        if (ctx.recoveryCase.attemptCount >= 3)
            score -= 0.2;
        // Customer segment bonus
        if (ctx.customer.segment === 'LOYAL')
            score += 0.1;
        if (ctx.customer.segment === 'HIGH_VALUE')
            score += 0.05;
        if (ctx.customer.segment === 'NEW')
            score -= 0.05;
        return score;
    }
    selectAction(ctx, probability) {
        if (probability < ctx.policy.minimumRecoveryProbability)
            return 'STOP';
        if (ctx.recoveryCase.attemptCount >= ctx.policy.maxAttempts)
            return 'STOP';
        const timeouts = [constants_1.FAILURE_TYPE.BANK_TIMEOUT, constants_1.FAILURE_TYPE.UPI_TIMEOUT, constants_1.FAILURE_TYPE.NETWORK_ERROR];
        const isTimeout = timeouts.includes(ctx.payment.failureType);
        if (isTimeout && ctx.recoveryCase.attemptCount === 0)
            return 'CREATE_PAYMENT_LINK';
        if (ctx.payment.failureType === constants_1.FAILURE_TYPE.CARD_EXPIRED)
            return 'REQUEST_CUSTOMER_ACTION';
        if (probability > 0.8)
            return 'CREATE_PAYMENT_LINK';
        if (probability > 0.65)
            return 'SEND_REMINDER';
        if (ctx.payment.amount > 500000)
            return 'ESCALATE_TO_HUMAN'; // > ₹5,000
        return 'SEND_REMINDER';
    }
    getDelay(failureType, action) {
        if (action === 'STOP')
            return 0;
    ...
    getPriority(amount, probability) {
        // High amount + high probability = critical priority
        if (amount > 1000000 && probability > 0.7)
            return 'CRITICAL'; // > ₹10,000
        if (amount > 500000 || probability > 0.8)
            return 'HIGH'; // > ₹5,000
        if (amount > 100000)
            return 'MEDIUM'; // > ₹1,000
        return 'LOW';
    }
    buildReason(ctx, probability, action) {
        const successRate = ctx.customer.totalPayments > 0
        ...
        parts.push(`Recommended action: ${action.replace(/_/g, ' ')} with ${Math.round(probability * 100)}% estimated recovery probability.`);
        return parts.join(' ');
    }
    getDecisionFactors(ctx, probability) {
        const factors = [];
        const successRate = ctx.customer.totalPayments > 0
            ? Math.round((ctx.customer.successfulPayments / ctx.customer.totalPayments) * 100)
            : 50;
        factors.push(`Customer payment success rate: ${successRate}% (${ctx.customer.successfulPayments}/${ctx.customer.totalPayments} payments)`);
