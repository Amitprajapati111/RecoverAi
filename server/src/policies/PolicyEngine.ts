...
export class PolicyEngine {
  evaluate(
    ...
    logger.info('PolicyEngine: evaluating AI decision', {
    ...
    if (recoveryCase.attemptCount >= policy.maxAttempts) {
      return this.block(
        `Maximum recovery attempts (${policy.maxAttempts}) reached`,
        policy
      );
    }
    ...
    if (aiDecision.recoveryProbability < policy.minimumRecoveryProbability) {
      return this.block(
        `Recovery probability ${(aiDecision.recoveryProbability * 100).toFixed(0)}% is below minimum threshold ${(policy.minimumRecoveryProbability * 100).toFixed(0)}%`,
        policy
      );
    }
    ...
    if (!policy.allowedActions.includes(aiDecision.recommendedAction)) {
      return this.block(
        `Action '${aiDecision.recommendedAction}' is not in merchant's allowed actions`,
        policy
      );
    }
    ...
    if (lastAttemptAt) {
      ...
      if (minutesSinceLastAttempt < policy.cooldownMinutes) {
        ...
        return this.block(
          `Cooldown period active. ${remainingMinutes} minutes remaining before next attempt`,
          policy
        );
      }
    }
    ...
    const requiresApproval =
      recoveryCase.amountAtRisk > policy.requireApprovalAboveAmount ||
  ...
  private block(reason: string, policy: IRecoveryPolicy): PolicyCheckResult {
    logger.info(`PolicyEngine: BLOCKED — ${reason}`);
    return {
      approved: false,
      requiresHumanApproval: false,
